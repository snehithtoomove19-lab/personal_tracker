      ...
      body: SafeArea(
        ...
        child: ListView(
          ...
          SwitchListTile(
            ...
            subtitle: const Text('Shows a quick popup when you open the app if anything needs attention (overdue tasks, unlogged mood, over-budget categories)'),
          ...
          const Padding(
            ...
            child: Text('AI Assistant', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13)),
          ),
          ...
          ListTile(
            ...
            subtitle: Text(
              app.aiApiKey.trim().isEmpty
                  ...
                  : '•••• ${app.aiApiKey.length > 4 ? app.aiApiKey.substring(app.aiApiKey.length - 4) : ''}',
            ),
          ...
          ListTile(
            ...
            subtitle: const Text('Saves transactions to a CSV file in app documents'),
            onTap: () async {
          ...
          ListTile(
            ...
            subtitle: const Text('Full backup of everything (transactions, tasks, notes, goals, mood, budgets)'),
            onTap: () => _showBackup(context, app),
          ),
          ListTile(
            leading: const Icon(Icons.restore_outlined),
            title: const Text('Restore from Backup'),
            subtitle: const Text('Paste a backup you copied earlier to restore it'),
            onTap: () => _showRestore(context, app),
          ),
          ListTile(
            leading: const Icon(Icons.cake_outlined),
            title: const Text('Birthday Contacts'),
            subtitle: const Text('Family and friends whose birthdays you want to remember'),
            onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const BirthdayContactsScreen())),
          ),
          const Divider(),
          SwitchListTile(
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: TextField(
          controller: ctrl,
          autofocus: true,
          decoration: const InputDecoration(hintText: 'e.g. gpt-4o-mini, gpt-4o'),
        ),
        actions: [
          TextButton(onPressed: () => Navigator.pop(ctx), child: const Text('Cancel')),
          FilledButton(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: SizedBox(
          ...
          child: Column(
            ...
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text('Saved to:\n${file.path}', style: const TextStyle(fontSize: 12)),
              const SizedBox(height: 12),
              const Text('You can also copy the backup text below and paste it into "Restore from Backup" later or on another device.'),
              const SizedBox(height: 12),
              Container(
                constraints: const BoxConstraints(maxHeight: 200),
                padding: const EdgeInsets.all(8),
                decoration: BoxDecoration(color: Colors.grey.shade200, borderRadius: BorderRadius.circular(8)),
                child: SingleChildScrollView(child: Text(jsonText, style: const TextStyle(fontSize: 10, fontFamily: 'monospace'))),
              ),
            ],
          ),
        ),
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: SizedBox(
          ...
          child: Column(
            ...
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              const Text('Paste your backup JSON below. This will replace all current data.',
              ...
              try {
                ...
                if (ctx.mounted) {
                  ScaffoldMessenger.of(ctx).showSnackBar(
                    const SnackBar(content: Text('Could not restore — invalid backup data')),
                  );
                }
              }
            },
  ...
  void _confirmErase(BuildContext context, app) {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text('This will permanently delete all transactions, tasks, notes, goals, and mood logs.'),
        actions: [
          TextButton(onPressed: () => Navigator.pop(ctx), child: const Text('Cancel')),
          TextButton(
            onPressed: () {
